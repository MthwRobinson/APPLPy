...
1. Start from `cgr.dev/chainguard/python`, and install the compiler/runtime packages with `apk add` so you are using Wolfi’s official package catalog (see https://github.com/wolfi-dev/os).
2. Use `pip install --upgrade pip setuptools wheel` and `python -m pip wheel .` to produce wheels without pulling runtime dependencies into the final image.
...
7. Use `make docker-build` from the `Makefile` to test that the updated docker build works
