...
class MarkovChain:
    ...
    def display(self, option="trans mat", n=1, method="float"):
        ...
        if option == "trans mat":
            ...
            if (not isinstance(n, int)) and (not isinstance(n, Symbol)):
    ...
    def matrix_convert(self, matrix):
        ...
        """
        display_mat = pd.DataFrame(matrix, index=self.state_space, columns=self.state_space)
        return display_mat
    ...
    def vector_convert(self, vector):
        ...
        """
        display_vec = pd.DataFrame(vector, index=self.state_space, columns=["Prob"])
        return display_vec
    ...
    def long_run_probs(self, method="float"):
        ...
        for item in self.classify:
            ...
            if can_reach == len(self.classify[item]):
                abs_flag = True
            if item != "Transient" and not abs_flag:
    ...
    def classify_states(self):
        ...
        state_dict = {}
        for k in range(c):
            state_dict[classes[k]] = [self.state_space[i] for i in range(n) if T[i] == k]
    ...
    def probability(self, states, given=None, method="float"):
        ...
        if isinstance(given, list):
            if given[0][0] < states[0][0]:
                ...
                total_states = given + states
                total_prob = self.probability(states=total_states, method=method)
        return total_prob
